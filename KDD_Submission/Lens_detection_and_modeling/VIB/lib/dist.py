...
class Normal(nn.Module):
    ...
    def _check_inputs(self, size, mu_logsigma,latent=True):
        ...
        if size is None and mu_logsigma is None:
            ...
            if (latent):
                ...
                logsigma = torch.clamp(mu_logsigma.select(-1, 1), min=-11.5129,max=2.302)
                
            else:
                mu = mu_logsigma
                logsigma = torch.Tensor(mu_logsigma.size()).fill_(-6.9077)
    ...
    def sample(self, size=None, params=None,latent=True):
        mu, logsigma = self._check_inputs(size, params,latent=latent)
        std_z = Variable(torch.randn(mu.size()).type_as(mu.data))
        sample = std_z * torch.exp(logsigma) + mu
        return sample

    def log_density(self, sample, params=None,latent=True):
        if params is not None:
            mu, logsigma = self._check_inputs(None, params,latent=latent)
        else:
            mu, logsigma = self._check_inputs(sample.size(), None,latent=latent)
            mu = mu.type_as(sample)
            logsigma = logsigma.type_as(sample)

        c = self.normalization.type_as(sample.data)
        inv_sigma = torch.exp(-logsigma)
        tmp = (sample* inv_sigma - mu* inv_sigma) 
        if utils.isnan(tmp).any():
            print ("NAN in tmp")
        if utils.isnan(sample*inv_sigma).any():
            print  ("NAN in sample")
        elif utils.isnan(mu* inv_sigma).any():
            print  ("NAN in mu")
        elif utils.isnan(sample*inv_sigma - mu*inv_sigma).any():
            print ("NAN in inv_sigma", (torch.max(sample*inv_sigma),
            torch.max(mu*inv_sigma),torch.max(sample),torch.max(mu),torch.max(inv_sigma)))
        return -0.5 * (tmp * tmp + 2 * logsigma + c)

    def NLL(self, params, sample_params=None,latent=True):
        ...
        if sample_params is not None:
            sample_mu, sample_logsigma = self._check_inputs(None, sample_params,latent=latent)
        else:
...
class Bernoulli(nn.Module):
    ...
    def sample(self, size=None, params=None,latent=False):
    ...
    def log_density(self, sample, params=None,latent=False):
        '''
    ...
    def Entropy(self, sample, params=None,latent=False):
        '''
        H[x] = −μ ln μ − (1 − μ) ln(1 − μ)
        '''
        presigm_ps = self._check_inputs(sample.size(), params).type_as(sample)
        p = (F.sigmoid(presigm_ps) + eps) * (1 - 2 * eps)
        entropy = -(p + eps) * torch.log(p + eps) - (1 - p + eps) * torch.log(1 - p + eps)
        return entropy 

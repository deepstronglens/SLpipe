...
class Phantom(srdata.SRData):
    def __init__(self, args, train=True):
        super(Phantom, self).__init__(args, train)
    ...
    def _scan(self):
        ...
        for i in range(idx_begin + 1, idx_end + 1):
            filename = np.str(i) #'{:0>4}'.format(i)
    ...
    def __len__(self):
        if self.train:
            #print ("Data_class Train", len(self.images_hr) * self.repeat)
            return len(self.images_hr) * self.repeat
        else:
            #print ("Data_class Train=false", len(self.images_hr) )
